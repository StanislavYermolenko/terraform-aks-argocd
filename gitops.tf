...
resource "null_resource" "gitops_instructions" {
  provisioner "local-exec" {
    command = <<-EOT
      ...
      echo "argocd repo add git@github.com:StanislavYermolenko/gitops-config.git --ssh-private-key-path ~/.ssh/gitops_key"
      echo ""
      echo "# 4. Create root application"
      echo "argocd app create root --repo git@github.com:StanislavYermolenko/gitops-config.git --path root --dest-server https://kubernetes.default.svc --dest-namespace argocd --sync-policy automated --auto-prune --self-heal"
      echo ""
